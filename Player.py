...
class Player(object):
    ...
    def player_physics(self, core):
        ...
        # Disparo de bola de fogo e movimento rápido
        self.fast_moving = False
        if core.keyShift:
        ...
        # removendo o erro computacional
        ...
        if not self.on_ground:
            # Movendo-se para cima, o botão é pressionado
            if (self.y_vel < 0 and core.keyU):
                self.y_vel += GRAVITY
                
            # Movendo para cima, o botão não é pressionado - salto baixo
        ...
        # parâmetro on_ground não será estável sem este pedaço de código
        ...
        # Verificação de borda do mapa
        if self.rect.y > 448:
            core.get_map().player_death(core)

        # Finalizar verificação de colisão de sinalizador
    ...
    def update_image(self, core):
        ...
        if self.powerLVL in (0, 1, 2):
            ...
            # Jogador decidiu seguir em outra direção, mas ainda não parou
